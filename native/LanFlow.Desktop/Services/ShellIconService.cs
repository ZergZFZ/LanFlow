...
public sealed class ShellIconService
{
    ...
    public ImageSource? GetIcon(string? path) =>
        string.IsNullOrWhiteSpace(path) ? null : _cache.GetOrAdd(path, ExtractIcon);
    ...
    private static ImageSource? ExtractIcon(string path)
    {
        try
        {
            ...
            if (!exists)
            {
                flags |= ShellFileInfoFlags.UseFileAttributes;
            }

            var result = SHGetFileInfo(iconPath, attributes, out var info, (uint)Marshal.SizeOf<ShellFileInfo>(), flags);
