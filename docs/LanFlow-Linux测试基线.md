...
### VM 验证记录（round5-b2.7，2026-08-10，任务栏图标延迟优化）

> 触发：用户反馈"任务栏图标出现和消失都会延迟"。量化定位后优化（MainWindow.axaml.cs，commit 待填）。

| 项 | 优化前 | 优化后 | 说明 |
|---|---|---|---|
| 唤回后点桌面消失 | ~2s | 227ms~600ms | **根因 bug**：`Opened` 事件每次 `Show()`（唤回）都把失焦抑制覆盖为 2s（本意是启动 2s），唤回后点桌面被 2s 抑制挡住 → 改为仅首次启动设 2s（`_startupSuppressDone`） |
| 失焦检测轮询 | 400ms | 150ms | 焦点离开窗口后隐藏更及时 |
| 唤回抑制期 | 800ms | 600ms | Topmost 抢焦 300ms + 300ms 余量；过短会"唤回即消失"（抢焦未稳被轮询误隐藏） |
| 隐藏后轮询 | 继续 X 查询 | `!IsVisible` 跳过 | 隐藏后零 X 开销，消除冗余 Hide 调用 |
| 出现（map） | ~5ms | ~5ms | 无变化（Dock 侧刷新非应用可控） |

> 验证要点：① 修复前日志取证：唤回后 tick 打到 2s 抑制过期才 `self=False→Hide`，点击外部后 ~1s 才 unmap；② 修复后回归：唤回后窗口保持可见（3 轮 2.5s 全过）、`hideOnDeactivate=false` 点桌面不隐藏、`=true` 点桌面隐藏+唤回可见全 PASS；③ 已知限制（Deepin WM 行为）：唤回抢焦期（~300ms）内点桌面偶发被 WM 吞焦（窗口不消失，需再点一次）；Topmost 抢焦成功率有平台波动，间歇性"唤回即消失"无法完全消除，600ms 抑制已将其窗口期压至最低。
